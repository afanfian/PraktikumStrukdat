...
void avl_remove(AVL *avl,unsigned long long value){
    if(avl_find(avl,value)){
...
void order1(AVLNode *root, unsigned long long coba, unsigned kedalaman) 
{
    if (root) 
    {
        order1(root->left,coba-1,kedalaman);
        if(coba==kedalaman)
        {
            if(x==2)
            {
        ...
        order1(root->right,coba-1,kedalaman);
    }
}
void order2(AVL *root, unsigned long long kedalaman) 
{
    total=0;
    unsigned long long Height=_getHeight(root->_root);
    order1(root->_root,Height,kedalaman);
    if(x==3)
    {
        printf("%llu",total);
    }
...
int main()
{
    ...
    while(Q--)
    {
        unsigned long long satux;
        scanf("%llu",&satux);
        x=satux;
        if(satux==1)
        {
            unsigned long long dua;
            scanf("%llu",&dua);
            avl_insert(&afan,dua);
        }
        else 
